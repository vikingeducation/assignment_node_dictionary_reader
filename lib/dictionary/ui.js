...
var selectDictionary = (dictFileStr) => {
  ...
  startListening();
  prompts.selectDictionaryPrompt(dictFileStr);
  ...
  var onData = (data) => {
    ...
    if((!isNaN(data)) && ((data <= dictFileArray.length) && (data > 0)) ) {
      ...
      prompts.displayDictionaryStats(currentDictionary, dictFileArray, data);
...
var dictFileStr = prompts.formatInitPrompt(dictFileArray);
selectDictionary(dictFileStr);
