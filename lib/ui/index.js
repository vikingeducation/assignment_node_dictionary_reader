...
function dictionaryLoad(path) {
  ...
  searchDialogue(dictionaryData.dataObj);
}
...
function searchListener(searchNum, data) {
  ...
  process.stdin.on('data', function(searchStr) {
    ...
    if (searchNum === '1') {
      ...
      //use regex instead of startswith()
      //search.partialSearch(term, data);
    } else if (str === '3') {
      //search.beginsWithSearch(term, data);
    } else if (str === '4') {
      //search.endsWithSearch(term,data);
    } else {
