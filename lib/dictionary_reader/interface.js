...
function one() {
  ...
  console.log('here is our array: ' + arr)
  welcomeMessage();
  dictsMessage(arr);
...
function two(choice) {
  ...
  var onData = (option) => {
    ...
    if ( Number.isInteger(parseInt(option)) && option < 5 && option > 0) {
      ...
      searchMessage(`Invalid: ${ option}. Please sepcify the number again.`);
    }
  };
...
function three(dictionary, option) {
  ...
  var onData = (phrase) => {
    ...
    if (typeof phrase == 'string') {
      ...
      process.stdin.pause();
      process.stdin.removeListener('data', onData);
      saver.prompter(results);
    } else if (option === 'q') {
      process.stdin.pause();
      process.stdin.removeListener('data', onData);
      console.log('Goodbye!');
    } else {
      searchMessage(`Invalid: ${ phrase }. Please sepcify the number again.`);
    }
  };
...
//     let jsonLibrary = [];
//
// 	var directories = new Promise((resolve, reject) => {
