  ...
  var onData = input => {
    ...
    if (input === 'q') {
      ...
      fs.readFile(`${path}${dictArray[parseInt(input) - 1]}`, (err, data) => {
        ...
        let displayArray = Object.keys(dictObj).sort();
...
let onSearch = (input, parsed) => {
  ...
  process.stdin.on('data', data => {
    ...
    let result = search(input, data, parsed);
    ...
    process.stdin.removeAllListeners('data');
    process.stdin.resume();
    console.log("Would you like to save your results?(y/n)");
    process.stdin.on('data', choice => {
      choice = choice.trim();
      if (choice === 'y') {
        console.log("What would you like to name this file?");
        process.stdin.pause();
        process.stdin.removeAllListeners();
        process.stdin.resume();
        process.stdin.on('data', fileName => {
          const saver = require('./save');
